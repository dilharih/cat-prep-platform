...
function validateQuestionData(data, { partial = false } = {}) {
  ...
  if (!partial || data.question !== undefined) {
    if (typeof data.question !== "string" || data.question.trim().length < 1 || data.question.trim().length > 10000) {
      throw validationError("Question text is required and must be at most 10000 characters");
    }
  }

  if (!partial || data.correctAnswer !== undefined) {
    if (typeof data.correctAnswer !== "string" || data.correctAnswer.trim().length < 1 || data.correctAnswer.trim().length > 500) {
      throw validationError("Correct answer is required");
    }
  }

  if (!partial || data.topic !== undefined) {
    if (typeof data.topic !== "string" || data.topic.trim().length < 1 || data.topic.trim().length > 200) {
  ...
  if (!partial || data.year !== undefined) {
    if (!Number.isInteger(data.year) || data.year < 1990 || data.year > 2100) {
      throw validationError("Year must be between 1990 and 2100");
    }
  }

  if (!partial || data.slot !== undefined) {
    if (!Number.isInteger(data.slot) || data.slot < 1 || data.slot > 3) {
      throw validationError("Slot must be between 1 and 3");
    }
  }

  if (data.marks !== undefined && (!Number.isInteger(data.marks) || data.marks < 0 || data.marks > 100)) {
    throw validationError("Marks must be between 0 and 100");
  }

  if (data.negativeMarks !== undefined && (!Number.isInteger(data.negativeMarks) || data.negativeMarks < 0 || data.negativeMarks > 100)) {
    throw validationError("Negative marks must be between 0 and 100");
  }

  if (data.passageContent !== undefined && data.passageContent !== null && (typeof data.passageContent !== "string" || data.passageContent.length > 30000)) {
    throw validationError("Passage must be at most 30000 characters");
  }

  if (data.explanation !== undefined && data.explanation !== null && (typeof data.explanation !== "string" || data.explanation.length > 10000)) {
    throw validationError("Explanation must be at most 10000 characters");
  }
  ...
  if (data.type === "MCQ" && !partial) {
    ...
    if (options.some((option) => typeof option !== "string" || option.trim().length === 0)) {
      throw validationError("All four options are required for MCQ questions");
    }
  }
}
...
function buildQuestionData(data, { partial = false } = {}) {
  ...
  for (const field of stringFields) {
    if (!partial || data[field] !== undefined) {
      result[field] = data[field] === null ? null : data[field].trim();
    }
  }
...
async function getMockTestQuestions(mockTestId) {
  ...
  return prisma.mockTestQuestion.findMany({
    where: { mockTestId },
    orderBy: { order: "asc" },
...
async function createQuestionForMockTest(mockTestId, data) {
  ...
  if (!Number.isInteger(questionData.year) || !Number.isInteger(questionData.slot)) {
    throw validationError("Year and slot are required for each question");
  }
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (data.passageContent?.trim()) {
      const passage = await tx.passage.create({
        data: { title: data.passageTitle?.trim() || null, content: data.passageContent.trim() },
      });
    ...
    await tx.mockTestQuestion.create({
      data: { mockTestId, questionId: question.id, order: (last?.order ?? -1) + 1 },
    });
...
async function updateQuestion(questionId, data) {
  ...
  const updated = await prisma.$transaction(async (tx) => {
    const question = await tx.question.update({ where: { id: questionId }, data: buildQuestionData(data, { partial: true }) });
